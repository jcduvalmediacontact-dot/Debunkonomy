---
chapitre: L11.C09
titre: "L'incidence — qui paie le reflux"
livre: 11
langue: fr
licence: CC-BY-SA-4.0
type: chapitre
statut: brouillon
revision_de_fond: 2026-09-13
autorite: preparatoire
citable: false
regime: conception
sources_primaires:
  - ref: S1
    nature: theorie
    reference: "J.-C. Duval, L'économie de l'équilibre, Debunk'Onomy, 2026, chapitre 9, p. 188-193 — les deux ressorts et leurs assiettes. Le premier frappe la consommation finale et les acquisitions d'actifs, modulé sur l'empreinte cumulée ; le second les encaisses inactives au-delà d'un SEUIL D'EXEMPTION INDEXÉ SUR LE REVENU MÉDIAN NATIONAL, assiette consolidée au bénéficiaire effectif. CE QUE LE CORPUS RELÈVE ICI, ET QUI EST LE POINT DE DÉPART DU CHAPITRE : le livre et le Cahier raisonnent en VOLUMES — combien il faut retirer — et ne posent nulle part la question de savoir SUR QUI le retrait pèse. Le mot incidence n'y figure pas"
    etat_lecture: a_requalifier
  - ref: S2
    nature: theorie
    reference: "J.-C. Duval, note de travail du 2026-09-05 ouvrant le Livre 11, et instruction de conception du même jour sur les dérogations — objectifs assignés au reflux et volonté que les prix des produits essentiels demeurent « stables et abondants ». CE QUI COMPTE POUR CE CHAPITRE : l'objectif d'abondance et de stabilité des essentiels est un objectif DISTRIBUTIF déguisé en objectif de prix, puisque la part du revenu consacrée aux essentiels décroît avec le revenu. Versée en L11.C01 [S1], L11.C03 [S3] et L11.C08 [S1]"
    etat_lecture: a_requalifier
  - ref: S3
    nature: theorie
    reference: "J. Couppey-Soubeyran, P. Delandre, A. Sersiron, Le pouvoir de la monnaie, Les Liens qui Libèrent, janvier 2024 — OUVERT le 2026-09-05. Leur prélèvement sur les stocks frappe les RÉSERVES BANCAIRES et non les encaisses des ménages, ce qui déplace entièrement la question d'incidence vers la répercussion bancaire ; leur prélèvement sur les flux frappe chaque paiement à 0,5 %, et les flux financiers à 0,05-0,2 % seulement. LE CORPUS RELÈVE QUE CETTE STRUCTURE DE TAUX A UNE CONSÉQUENCE DISTRIBUTIVE NON DISCUTÉE PAR SES AUTEURS : elle impose plus lourdement l'achat d'un bien que la transaction financière"
    etat_lecture: a_requalifier
  - ref: S4
    nature: jurisprudence
    reference: "Cour de justice de l'Union européenne, arrêt de novembre 2022 sur les registres de bénéficiaires effectifs — versé en L1.C21 § 6, L11.C02 [S4] et L11.C07 [S3]. Retenu ici pour sa conséquence distributive : à défaut de consolidation, le fractionnement des soldes échappe au demurrage, et le fractionnement suppose des moyens. RÉSERVE : arrêt non ouvert de première main ; sa portée exacte est la vérification la plus rentable du dossier"
    etat_lecture: a_requalifier
  - ref: S5
    nature: theorie
    reference: "A. L. Bovenberg, R. A. de Mooij, American Economic Review, 84(4), 1994 ; D. Fullerton, G. E. Metcalf, Chicago-Kent Law Review, 73(1), 1997 — érosion de l'assiette par le succès de l'instrument. Déjà au dossier en L1.C21 [S10], où elles fondent F1. RÉSERVE : non ouvertes de première main"
    etat_lecture: a_requalifier
verifications_en_attente:
  # ── Méthode ────────────────────────────────────────────────────────────────
  # CE CHAPITRE N'IMPORTE PRESQUE AUCUNE SOURCE NOUVELLE. Il consolide ce que
  # quatre chapitres du Livre 11 ont trouvé séparément, et pose une question
  # que ni le livre ni le Cahier ne posent : qui supporte le reflux. Le corpus
  # le fait à ce stade parce que la réponse commande le verdict, et parce que
  # quatre convergences indépendantes ne se traitent plus une par une.
  #
  # SA FAIBLESSE EST AILLEURS QUE DANS LES SOURCES : elle est dans les DONNÉES.
  # Deux paragraphes reposent sur des grandeurs empiriques que le corpus n'a
  # pas — l'élasticité de l'empreinte au revenu au § 3, la composition du
  # patrimoine par décile au § 4. Les deux sont disponibles et non ouvertes.
  # Le chapitre énonce donc des CRITÈRES vérifiables, non des conclusions.
  #
  - "L'ÉLASTICITÉ DE L'EMPREINTE AU REVENU, et c'est la grandeur qui décide du
     § 3. Un prélèvement sur la consommation finale est régressif par nature,
     mais il est ici modulé sur l'empreinte. **Le signe net de l'incidence
     dépend donc d'une seule question : quand le revenu double, l'empreinte de
     la consommation fait-elle plus ou moins que doubler ?** Si elle fait plus,
     la modulation renverse la régressivité et l'instrument est progressif. Si
     elle fait moins, elle l'atténue sans la renverser. Cette grandeur existe
     dans la littérature sur les inégalités d'émissions et **le corpus ne l'a
     pas ouverte** — c'est la même acquisition que celle demandée en L11.C03
     § 5 pour l'effet de position."
  - "LA COMPOSITION DU PATRIMOINE PAR DÉCILE, et elle décide du § 4. Le corpus
     y soutient que le demurrage frappe les patrimoines moyens plutôt que les
     grandes fortunes, celles-ci détenant peu d'encaisses et beaucoup d'actifs.
     **C'est une proposition empirique, vraisemblable et non vérifiée.** La
     donnée existe dans les enquêtes patrimoine des banques centrales et des
     instituts statistiques. Tant qu'elle n'est pas produite, le § 4 énonce une
     hypothèse et le corpus le marque comme telle."
  - "L'incidence du prélèvement sur les réserves bancaires [S3]. Si le nœud
     retenu était celui des voisins, toute la question d'incidence se
     déplacerait vers la répercussion bancaire — les banques répercutent,
     absorbent ou réduisent leurs réserves. **La littérature sur la
     répercussion des taux négatifs sur les déposants existe et traite
     exactement ce point** ; le corpus ne l'a pas ouverte, et A12 ne peut pas
     être tranché sans elle."
  - "Le seuil d'exemption pour les PERSONNES MORALES. Relevé en L11.C08 et non
     traité ici : le seuil indexé sur le revenu médian n'a aucun sens appliqué
     à la trésorerie d'exploitation d'une entreprise. Un seuil unique
     exempterait les grandes trésoreries oisives ou frapperait les fonds de
     roulement ordinaires. **Le dispositif n'a pas de seuil pour les personnes
     morales et il lui en faut un** — c'est un manque de conception, pas un
     paramètre."
  - "OUVERTE — les corrections d'incidence, qui sont procédurales et non
     tarifaires. Le § 6 les énumère sans les instruire. Portées en arbitrage
     A14."
  - "**LES DEUX RESSORTS DU REFLUX NE DÉSIGNENT PAS LE MÊME DÉBITEUR, ET CE
     CHAPITRE NE L'A PAS POSÉ.** Versé le 2026-09-08 par L19.C05. **Le demurrage
     frappe celui qui DÉTIENT** — il est, sous la norme, un intérêt dû par le détenteur
     à proportion de sa position [L19.C03]. **Le reflux transactionnel frappe celui
     qui TRANSACTE.** **Ce ne sont ni les mêmes personnes, ni la même assiette, ni la
     même nature comptable** — revenu de la propriété d'un côté, transfert de l'autre.
     **L'incidence instruite ici doit donc être reprise instrument par instrument**, et
     non pour « le reflux » pris comme un tout."
resume: "Ce chapitre pose une question que ni le livre ni le Cahier ne posent : qui supporte le reflux. Le dispositif raisonne en volumes — combien retirer — et jamais en incidence. Le chapitre consolide d'abord ce que quatre chapitres du Livre 11 ont trouvé séparément et par des chemins indépendants : la cascade favorise les ensembles intégrés, la valeur par défaut favorise ceux qui savent documenter, la consolidation par bénéficiaire effectif favorise ceux qui disposent de structures, la dérogation favorise ceux qui disposent d'un conseil. Quatre biais, une seule direction. Il établit ensuite que l'incidence du premier ressort n'est pas déterminée a priori : un prélèvement sur la consommation finale est régressif par nature, mais il est ici modulé sur l'empreinte, de sorte que le signe net dépend d'une seule grandeur — l'élasticité de l'empreinte au revenu — que le corpus n'a pas et qui est disponible. Il établit pour le second ressort une hypothèse plus inquiétante et non vérifiée : le demurrage frappe les encaisses, or les grandes fortunes détiennent peu d'encaisses et beaucoup d'actifs, tandis que les patrimoines moyens détiennent l'inverse ; l'instrument atteindrait donc le milieu plutôt que le sommet, et la dérogation aggraverait cet effet puisque l'accès aux produits exemptés suppose un conseil. Il relève que les quatre biais sont tous de nature procédurale et jamais tarifaire, ce qui a une conséquence pratique décisive : aucune correction par les taux ne les atteint. Il conclut que le dispositif, conçu contre la concentration du pouvoir financier, favoriserait dans sa forme actuelle les acteurs concentrés — non par contradiction de principe, mais par accumulation de mécanismes dont aucun n'a été choisi pour cela."
concepts: [reflux_collectif, reflux_transactionnel, demurrage, bareme, degeneratif, qualification_regenerative]
renvois: [L1.C15, L1.C21, L1.C22, L11.C03, L11.C05, L11.C06, L11.C07, L11.C08, L19.C05]
---

# L'incidence — qui paie le reflux

::etat:: Ce chapitre pose une question que ni le livre ni le Cahier Technique ne posent. Ils établissent **combien** il faut retirer de monnaie ; ils ne demandent nulle part **sur qui** le retrait pèse [S1]. Le mot n'y figure pas, et l'absence n'est pas une négligence de rédaction : c'est le passage manquant entre un mécanisme monétaire et une proposition politique.

## 1. Quatre chapitres ont trouvé la même chose sans se concerter

::etat:: Le corpus ne l'aurait retenu d'aucun d'eux pris isolément. Il le retient de leur convergence.

| Où | Mécanisme | Qui il favorise |
|---|---|---|
| L11.C05 § 4 | La cascade : la charge croît avec le nombre d'intermédiations | les **ensembles intégrés**, qui suppriment les transactions internes |
| L11.C06 § 2 | La valeur par défaut punitive : on en sort en documentant | ceux qui savent **documenter** |
| L1.C21 § 6 | La consolidation par bénéficiaire effectif, fermée en droit | ceux qui disposent de **structures** |
| L11.C08 § 4 | La dérogation : on y accède en plaçant au bon endroit | ceux qui disposent d'un **conseil** |

::hypothese:: **Quatre mécanismes sans rapport les uns avec les autres, une seule direction.** Aucun n'a été choisi pour cela ; chacun résout un problème réel et différent. C'est ce qui rend la convergence sérieuse : elle n'est pas un défaut d'intention, elle est un effet d'accumulation, et les effets d'accumulation ne se voient pas dans l'examen pièce à pièce.

## 2. Le premier ressort : régressif par nature, corrigé par la modulation

::etat:: Un prélèvement assis sur la **consommation finale** est régressif au sens standard, pour une raison qui ne dépend d'aucune hypothèse : la part du revenu consacrée à la consommation décroît lorsque le revenu croît. À taux uniforme, il pèse davantage, en proportion du revenu, sur les revenus modestes.

::etat:: **Mais le taux n'est pas uniforme** — c'est tout l'objet du dispositif. Il est modulé sur l'empreinte du bien [S1]. La question n'est donc pas de savoir si un prélèvement sur la consommation est régressif : elle est de savoir si la modulation renverse la régressivité, ou seulement l'atténue.

::hypothese:: **Et cela se ramène à une seule grandeur, que le corpus peut nommer sans la connaître.**

> Quand le revenu double, l'empreinte de la consommation fait-elle **plus** ou **moins** que doubler ?

::hypothese:: **Si elle fait plus** — si l'empreinte croît plus vite que le revenu —, la modulation renverse la régressivité et l'instrument devient progressif. **Si elle fait moins**, la modulation l'atténue sans la renverser, et le prélèvement demeure régressif malgré son intention.

::hypothese:: **Le corpus ne connaît pas la réponse et refuse de la deviner.** Il note seulement que la grandeur existe dans la littérature sur les inégalités d'émissions, qu'elle est la même que celle demandée en L11.C03 § 5 pour l'effet de position, et qu'**une seule acquisition réglerait les deux questions.** Il note aussi que cette grandeur décide de la nature politique du dispositif, ce qui est beaucoup pour un nombre que personne n'a cherché.

::hypothese:: Une remarque sur les exemptions, qui joue en faveur du dispositif : l'exemption des produits essentiels est **le correcteur de régressivité le mieux établi qui soit**, puisque la part des essentiels dans le budget décroît fortement avec le revenu. L'objectif de l'auteur que les essentiels demeurent « stables et abondants » [S2] est donc, sans qu'il le formule ainsi, **un objectif distributif déguisé en objectif de prix** — et c'est le plus efficace des correcteurs dont il dispose.

## 3. Le second ressort : l'hypothèse inquiétante

::etat:: Le demurrage frappe les **encaisses** — la monnaie détenue [S1]. À première vue il est progressif : les soldes monétaires sont plus concentrés que les revenus, et un seuil d'exemption indexé sur le revenu médian met les ménages modestes hors d'atteinte.

::hypothese:: **Le corpus soutient que cette lecture est probablement fausse, et il marque immédiatement qu'il ne l'a pas vérifiée.** Les grandes fortunes ne détiennent pas leur patrimoine en encaisses. Elles le détiennent en titres, en immobilier, en parts de sociétés, en fonds — c'est-à-dire en **actifs**, que le demurrage ne touche pas. Les encaisses importantes se trouvent plutôt chez le ménage aisé sans être fortuné, chez celui qui a vendu un bien, chez celui qui constitue un apport, chez la petite entreprise qui garde de la trésorerie.

::hypothese:: **Si c'est exact, le demurrage atteint le milieu et manque le sommet** — l'inverse de ce que le dispositif poursuit, et l'inverse de ce que sa présentation laisse attendre. Le corpus insiste : **c'est une proposition empirique, vraisemblable et non vérifiée.** La donnée existe dans les enquêtes patrimoine ; elle n'a pas été ouverte. Tant qu'elle ne l'est pas, ce paragraphe est une hypothèse marquée comme telle.

::hypothese:: **Et la dérogation aggraverait cet effet plutôt que de le corriger.** L11.C08 a établi qu'elle offre une porte de sortie ; encore faut-il savoir qu'elle existe et savoir y entrer. **Une porte accessible à qui dispose d'un conseiller est une porte qui trie.** Le ménage aisé sans conseil paie ; le patrimoine administré ne paie pas. Le correctif de justice que la dérogation apporte au principe se retourne dans son application, à moins que l'accès n'en soit rendu trivial — ce qui est une exigence de conception, non un vœu.

## 4. Les quatre biais sont procéduraux, et c'est ce qui compte

::hypothese:: Le corpus relève une propriété commune aux quatre mécanismes du § 1, et elle a une conséquence pratique décisive. **Aucun d'eux ne passe par le taux.**

::hypothese:: La cascade dépend du **nombre de transactions**. La valeur par défaut dépend du **coût de la procédure de révision**. La consolidation dépend d'un **régime juridique**. La dérogation dépend de l'**accès à l'information et au conseil**. Ce sont quatre paramètres de procédure, et le barème ne les contient pas.

::hypothese:: **Conséquence : aucune correction tarifaire ne les atteint.** On peut baisser tous les taux, en modifier la dispersion, exempter davantage — les quatre biais demeurent identiques, parce qu'ils ne portent pas sur ce qu'on paie mais sur qui se trouve dans l'assiette et à quelles conditions on en sort. **C'est pourquoi ils sont invisibles dans le document qu'on discute**, qui est un tableau de taux.

::hypothese:: Cela confirme sur un autre terrain ce que L11.C06 § 4 avait établi : **un barème est trois objets — une table, une procédure, une charge de la preuve — et le livre n'en écrit qu'un.** Le § 1 du présent chapitre montre que le deuxième et le troisième portent toute l'incidence.

## 5. Ce que cela fait au verdict, et ce que cela ne fait pas

::hypothese:: Le corpus doit énoncer le résultat sans l'adoucir. **Un dispositif conçu contre la concentration du pouvoir financier favoriserait, dans sa forme actuelle, les acteurs concentrés.** Non par contradiction de principe — l'intention est l'inverse — mais par accumulation de mécanismes dont aucun n'a été choisi pour cela.

::hypothese:: **Ce que cela ne fait pas : cela ne réfute pas la thèse.** Chacun des quatre mécanismes a une correction connue, et aucune n'exige d'abandonner l'instrument. Une procédure de révision gratuite et rapide neutralise le deuxième. Une assiette sur la consommation finale, que le livre retient déjà, supprime le premier. Un seuil pour les personnes morales, qui manque, traite une part du troisième. Un accès à la dérogation rendu automatique plutôt que déclaratif traite le quatrième.

::hypothese:: **Ce que cela impose : les corrections sont procédurales, donc elles doivent être écrites dans le dispositif et non renvoyées à sa mise en œuvre.** Une procédure décrite comme un détail d'application est une procédure que personne n'arbitrera, et le § 4 vient d'établir qu'elle porte l'incidence. Porté en **arbitrage A14**.

::hypothese:: Le corpus relève enfin ce qui manque à ce chapitre pour conclure, et ce n'est pas une source : **ce sont deux nombres.** L'élasticité de l'empreinte au revenu, qui décide du signe du premier ressort. La composition du patrimoine par décile, qui décide de la cible du second. **Les deux existent, aucun n'a été cherché, et ils décident ensemble de la nature politique du dispositif.** C'est le manque le plus coûteux que le Livre 11 ait rencontré, parce qu'il ne se comble pas par un arbitrage — il se comble par une lecture.

## 6. Portée

::etat:: **La question de l'incidence n'a jamais été posée.** Le livre et le Cahier raisonnent en volumes ; le mot ne figure nulle part. C'est le passage manquant entre un mécanisme monétaire et une proposition politique.

::hypothese:: **Quatre mécanismes indépendants poussent dans la même direction** — cascade, valeur par défaut, consolidation, dérogation — et favorisent les acteurs intégrés, documentés, structurés et conseillés. Aucun n'a été choisi pour cela ; c'est un effet d'accumulation, et les effets d'accumulation ne se voient pas pièce à pièce.

::hypothese:: **L'incidence du premier ressort n'est pas déterminée a priori.** Régressif par nature, modulé sur l'empreinte, son signe net dépend d'une seule grandeur : l'élasticité de l'empreinte au revenu. Le corpus la nomme et ne la connaît pas.

::hypothese:: **Celle du second est probablement l'inverse de ce qu'on attend.** Le demurrage frappe les encaisses, et les grandes fortunes n'en détiennent guère. L'instrument atteindrait le milieu et manquerait le sommet — hypothèse vraisemblable, non vérifiée, et la donnée existe.

::hypothese:: **Les quatre biais sont procéduraux et jamais tarifaires**, de sorte qu'aucune correction par les taux ne les atteint. C'est pourquoi ils sont invisibles dans un tableau de taux, qui est le seul document que le dispositif publie.

::hypothese:: **Le verdict provisoire est sévère et il n'est pas fatal.** Un dispositif conçu contre la concentration financière favoriserait les acteurs concentrés. Chacun des quatre mécanismes a une correction connue, et toutes sont procédurales — donc à écrire dans le dispositif, non à renvoyer à sa mise en œuvre. Arbitrage A14. Et il manque deux nombres, que personne n'a cherchés, qui décident ensemble de la nature politique de l'ensemble.

## BALAYAGE DE L'ARBITRAGE A35 — 2026-09-07

::etat:: **Annotation portée après l'arbitrage A35, rendu par l'auteur.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'arbitrage A35 renvoie à ce chapitre une question qu'il n'avait pas posée sous cette forme** [L10.C06]. L'auteur a arrêté le 2026-09-07 que **« c'est la société entière qui porte cette dette »**, et aussitôt que **« le choix est donné aux agents »**, la fonte étant **« graduellement plus forte »** selon l'impact de la consommation.

::hypothese:: **Le corpus enregistre une précision, et il la porte sans en faire une objection.** **Si la charge est modulée par la décision de chacun, la dette est collective PAR SA CAUSE et individuelle PAR SON INCIDENCE.** **Ce n'est pas une contradiction : c'est la structure d'un impôt pigouvien**, dont l'assiette est un comportement et le motif un dommage commun. **Mais la formule « portée par l'ensemble de la société » décrit le MOTIF, non le PORTEUR.**

::etat:: **La question rendue à ce chapitre est donc précise** : **qui paie effectivement une fonte graduée sur l'impact, une fois la répercussion prise en compte ?** Ce chapitre avait déjà relevé qu'une structure de taux imposant plus lourdement l'achat d'un bien que la transaction financière **a une conséquence distributive non discutée par ses auteurs**.

::hypothese:: **Et une vérification s'impose, parce que le motif invoqué par l'arbitrage est une catégorie proposée par le corpus.** **L1.C15 définit l'essentiel insolvable sans en établir l'ampleur ni l'absence d'autres financements** ; A35 s'en réclame pour écarter la rentabilité. **Il faut donc vérifier que l'assiette de la fonte ne reproduit pas le filtre que cette catégorie décrit** — une gradation sur l'impact frappe d'autant plus qu'on ne peut pas choisir, et **A34 a établi qu'un barème qui dit vrai exclut ceux qui ne peuvent pas payer.**

## BALAYAGE DU LIVRE 21, TROISIÈME — 2026-09-07

::etat:: **Annotation portée après L21.C04, qui ferme l'acquisition sur le ratio de levier de Bâle par lecture directe du texte.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **Une incidence de plus apparaît, et ce chapitre ne l'avait pas** [L21.C04]. **Le dispositif impose une charge en capital aux BANQUES DES PRESTATAIRES**, à hauteur de 3 % de l'encours de réserves créées [LEV20.7, LEV30.8] — **charge qu'aucune étape du dispositif ne leur demande d'accepter et qu'aucun barème ne compense.**

::hypothese:: **La question propre à ce chapitre se pose donc à nouveau, un cran plus haut.** **Une banque à sa limite de levier répercute, absorbe, ou refuse.** **Répercuter** signifie facturer le dépôt du prestataire, donc **prélever sur le paiement régénératif lui-même** ; **absorber** signifie lever du capital pour un service non demandé ; **refuser** signifie que le prestataire ne trouve pas de banque, ou n'en trouve qu'une petite. **Aucun des trois n'est le comportement visé**, et c'est exactement la structure que ce chapitre avait établie pour le prélèvement sur les réserves.

::hypothese:: **Réserve qui limite la portée** : les réserves étant pondérées à 0 % [CRE20.7], **une banque dont la contrainte mordante est le ratio pondéré héberge ces dépôts gratuitement.** **L'incidence ne se produit que chez les banques déjà contraintes par le levier — les plus grandes.**

## BALAYAGE DU LIVRE 22, CINQUIÈME — 2026-09-07

::etat:: **Annotation portée après L22.C05, qui instruit le troisième instrument de reflux par son assiette la plus grande.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **UNE INCIDENCE D'UN TYPE QUE CE CHAPITRE N'A PAS EXAMINÉ : CERTAINE, INDIVIDUELLE ET DIFFÉRÉE** [L22.C05]. **OCDE, « Pension Markets in Focus 2025 »** : les régimes à prestations définies, où l'employeur « **GUARANTEE future benefits** », ne pèsent plus que **31,9 % des actifs de retraite à fin 2024, contre 39,7 % dix ans plus tôt** — et le mouvement « **is ongoing** ».

::hypothese:: **Dans un régime garanti, une perte sur les actifs est absorbée par le garant. Dans un régime à cotisations définies, IL N'Y A PAS DE GARANT : la perte est intégralement supportée par le futur retraité.** **Un prélèvement sur les encours de retraite frappe donc, pour plus des deux tiers des actifs, DES INDIVIDUS SANS AUCUN TAMPON — et la part sans tampon augmente chaque année.**

::etat:: **C'est le CONTRAIRE de ce que ce chapitre a établi pour les autres instruments**, où l'incidence était incertaine, dispersée et susceptible d'être absorbée par un intermédiaire. **Le seul instrument capable de fermer la fuite du reflux est celui dont l'incidence est la plus directe et la moins amortie.**

## BALAYAGE DU LIVRE 25 — 2026-09-07

::etat:: **Annotation portée à l'ouverture du Livre 25, après lecture directe du Pacte international relatif aux droits économiques, sociaux et culturels.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'INCIDENCE QUE CE CHAPITRE ÉTUDIE DEVIENT, DANS CERTAINS CAS, UNE QUESTION DE DROIT** [L25.C01]. **L22.C02** a établi que l'assiette de consommation finale laisse échapper l'épargne et **frappe donc proportionnellement plus ceux qui dépensent tout ce qu'ils reçoivent.** **Le Pacte de 1966 protège le niveau de vie suffisant, l'alimentation et le logement, et son article 4 n'admet de limitation que par la loi, compatible avec la nature du droit, et pour le bien-être général dans une société démocratique.**

::hypothese:: **L'incidence régressive d'un reflux transactionnel n'est donc pas seulement un défaut de conception à corriger : c'est un motif d'opposabilité** dans les États parties. **Le corpus enregistre que la question « qui paie le reflux » a désormais une réponse partielle en droit : pas ceux dont cela entamerait les moyens de subsistance.**

## BALAYAGE DU LIVRE 25, QUATRIÈME — 2026-09-07

::etat:: **Annotation portée après L25.C04, qui ouvre le règlement instituant le Fonds social pour le climat de l'Union.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'INCIDENCE PEUT SE TRAITER PAR LE DROIT PLUTÔT QUE PAR L'ESPÉRANCE, ET UN RÈGLEMENT LE FAIT** [L25.C04]. **Règlement (UE) 2023/955, article 9 § 2** : « Les États membres prévoient **les dispositions légales et contractuelles nécessaires pour garantir que L'INTÉGRALITÉ DU BÉNÉFICE EST TRANSFÉRÉE** aux ménages vulnérables, aux microentreprises vulnérables ou aux usagers vulnérables des transports. »

::hypothese:: **Ce chapitre établit que l'incidence d'un prélèvement ne se lit pas sur celui qui le verse, et que le bénéfice d'une aide ne se lit pas sur celui qui la reçoit.** **Le règlement n'espère pas que le bénéfice passe : IL OBLIGE À L'ASSURER PAR CONTRAT.** **C'est une technique, et le corpus n'en avait aucune.**

::etat:: **Le considérant 11 énonce l'incidence que ce chapitre cherchait à caractériser** : la hausse des prix affecte « **de manière disproportionnée** » les ménages « **qui consacrent une grande partie de leurs revenus à l'énergie et aux transports** », qui « **n'ont pas accès à des solutions [...] alternatives et abordables** » et qui « **pourraient ne pas disposer des moyens financiers pour investir** ». **RÉSERVE : le règlement AFFIRME cette incidence, il ne la CHIFFRE pas, et le corpus n'a ouvert aucune étude qui le fasse.**
