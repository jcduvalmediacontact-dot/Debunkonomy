---
chapitre: L11.C02
titre: "Le trajet complet de la valeur"
livre: 11
langue: fr
licence: CC-BY-SA-4.0
type: chapitre
statut: brouillon
revision_de_fond: 2026-09-07
autorite: preparatoire
citable: false
regime: conception
sources_primaires:
  - ref: S1
    nature: theorie
    reference: "J.-C. Duval, arbitrage adressé au corpus le 2026-09-05 — « une grande coalition entre le GAÏA Economic Symposium et les banques centrales du monde entier », les activités définies par le GES occasionnant l'émission d'allocations « convertibles par la banque centrale en monnaie de banque centrale à finalités extra-financières » ; comptabilité des allocations « en miroir » entre le GES et les banques centrales ; reflux « en cascades vers les banques centrales puis des banques centrales vers le GES » ; et le point décisif : le GES peut « capter (accumuler) plus d'avoirs en comptabilités de monnaie nationales qu'elle n'a émis » d'allocations. Versé au dossier en L11.C01 [S6]"
    date_verification: 2026-09-05
  - ref: S2
    nature: theorie
    reference: "J.-C. Duval, Cahier technique, épisode 9 — écritures de l'émission aux trois étages : « les [NGA] entrent à l'actif du bilan de la banque centrale nationale comme actif transitoire, la monnaie nationale émise au bénéfice du prestataire sort au passif ». CONVENTION DE CITATION : les crochets marquent la substitution du sigle abandonné"
    date_verification: 2026-09-05
  - ref: S3
    nature: theorie
    reference: "J. Couppey-Soubeyran, P. Delandre, A. Sersiron, Le pouvoir de la monnaie. Transformons la monnaie pour transformer la société, Les Liens qui Libèrent, janvier 2024 — OUVERT DE PREMIÈRE MAIN le 2026-09-05, ouvrage procuré par l'autrice. Proposition voisine chiffrée, seule borne empirique dont le corpus dispose. DEUX PRÉLÈVEMENTS QUI SE DÉDOUBLENT : contribution monétaire sur les stocks et sur les flux, et compensations écologiques. Sur les STOCKS : « une contribution de l'ordre par exemple de 0,1 % sur les soldes de fin de mois des réserves bancaires déposées à la banque centrale (fin février 2023, ces réserves s'élevaient à environ 4 300 milliards d'euros, ce qui permettrait une ressource fiscale mensuelle de 4,3 milliards) », assimilable à « un taux d'intérêt négatif sur les réserves, tel que le pratiquait la BCE avant la crise sanitaire (à −0,5 %) ». Sur les FLUX : « un micro-impôt de 0,5 % prélevé sur chaque paiement », et une taxe de type Tobin de « 0,05 % à 0,2 % sur les flux financiers ». POINT DE CONCEPTION DÉCISIF pour ce chapitre : leur prélèvement sur les stocks frappe les RÉSERVES BANCAIRES déposées à la banque centrale, non les encaisses des détenteurs finaux"
    date_verification: 2026-09-05
  - ref: S4
    nature: jurisprudence
    reference: "Cour de justice de l'Union européenne, arrêt de novembre 2022 — l'accès du grand public aux informations sur les bénéficiaires effectifs constitue une ingérence grave dans les droits garantis par les articles 7 et 8 de la Charte, non limitée au strict nécessaire ni proportionnée. Déjà versée au dossier en L1.C21 § 6, où elle établit que la consolidation permanente des soldes par détenteur effectif — que le demurrage du dispositif suppose — se heurte à un obstacle de droit et non d'administration"
    date_verification: 2026-09-04
  - ref: S5
    nature: theorie
    reference: "Soldes TARGET2 de l'Eurosystème — créances et engagements entre banques centrales nationales et Banque centrale européenne nés des paiements transfrontaliers, sans échéance ni mécanisme de règlement prévu, et dont le statut a fait l'objet d'une controverse publique majeure au tournant des années 2010. ANALOGIE DU CORPUS, NON DES SOURCES : elle est proposée ici pour établir qu'une banque centrale peut porter durablement une créance perpétuelle sur une contrepartie institutionnelle, et que cette situation est politiquement explosive. RÉSERVE : aucune source n'a été ouverte sur ce point ; l'analogie est à vérifier avant tout emploi, et son ampleur chiffrée n'est pas reprise"
    date_verification: 2026-09-05
verifications_en_attente:
  # ── Méthode ────────────────────────────────────────────────────────────────
  # Ce chapitre est écrit APRÈS l'arbitrage du 2026-09-05 : il expose le régime
  # retenu, non une alternative. Il prolonge L11.C01 § 4 et prépare la question
  # que cet arbitrage a ouverte — que fait l'institution de ce qu'elle
  # accumule ?
  #
  # LIVRE DE CONCEPTION. Aucune source ne préexiste sur le circuit complet ; le
  # Cahier expose les écritures, non leur bouclage en régime permanent. La
  # discipline appliquée est celle de corpus/livres.yaml : les contraintes déjà
  # établies tiennent lieu de source, et la passe doit rendre le dispositif
  # plus falsifiable.
  #
  - "L'analogie TARGET2 [S5]. Elle porte le § 4, qui est le résultat le plus
     lourd du chapitre, et elle n'est adossée à aucune source ouverte. Il faut
     établir : le régime juridique exact de ces soldes, leur rémunération, ce
     qui a été soutenu dans la controverse et par qui, et surtout ce qui
     distingue une créance intra-système d'une créance sur une institution
     mondiale extérieure au système. **Une objection porteuse ne doit pas
     reposer sur une analogie non vérifiée** — c'est la règle que le corpus
     s'est donnée le 2026-09-04."
  - "Le volume des paiements soumis au prélèvement sur les flux. Le § 5 établit
     que le rendement du reflux transactionnel se calcule, et que personne ne
     l'a calculé. Le taux existe (0,5 % chez les voisins), l'assiette non. Il
     faut le volume annuel des paiements d'une zone monétaire, et la part qui
     resterait après exemption des essentiels et des régénératifs. Renvoi au
     Livre 13, et c'est la boucle courte annoncée."
  - "Le régime comptable des allocations à l'actif d'une banque centrale. Le
     § 4 soutient qu'une banque centrale porterait une créance perpétuelle non
     remboursable. Il faut confronter cette lecture aux normes applicables aux
     bilans de banques centrales et aux statuts existants, qui encadrent la
     nature des actifs éligibles."
  - "Le prélèvement sur les réserves bancaires est-il transposable ? Le § 5
     relève que les voisins évitent l'obstacle juridique du demurrage en
     frappant les réserves plutôt que les encaisses. Il faut établir ce que
     cette substitution coûte : l'incidence n'est pas la même, et la question
     de savoir si les banques la répercutent, l'absorbent ou la contournent
     décide de l'effet réel."
resume: "Ce chapitre trace le circuit complet de la valeur dans le régime que l'auteur a retenu le 2026-09-05 : une coalition à deux étages où l'institution mondiale émet des allocations que les banques centrales nationales convertissent en monnaie, et où le reflux remonte jusqu'à elle et y demeure. Il établit d'abord les écritures aux trois étages, puis suit ce qui s'accumule dans le temps. Il en tire quatre résultats. Le premier est que l'institution émettrice opère nécessairement en fonds propres négatifs entre l'émission et le reflux. LE CHAPITRE PRÉSENTAIT CE RÉSULTAT COMME LE RÉGIME ORDINAIRE D'UN ÉMETTEUR ; LA REVUE CONTRADICTOIRE DU 2026-09-07 A ÉTABLI QUE C'EST FAUX, une banque centrale qui achète un titre voyant son actif et son passif croître simultanément. L'écriture demeure, sa justification tombe : un émetteur ordinaire acquiert en émettant, celui-ci donne en émettant, et c'est la singularité même du dispositif. Le Livre 21 en a tiré la conséquence normative — le principe 15 du standard des infrastructures de marché exige des fonds propres positifs et liquides. Le deuxième est ce que porte réellement une banque centrale nationale : une créance perpétuelle, sans échéance ni mécanisme de règlement, sur une institution extérieure à son système. Le corpus propose une analogie avec les soldes intra-européens pour établir que la chose est praticable et politiquement explosive, et il la donne pour ce qu'elle est, une analogie non vérifiée. Le troisième porte sur le point du circuit où le reflux est prélevé : la proposition voisine la plus proche frappe les réserves bancaires déposées à la banque centrale plutôt que les encaisses des détenteurs, ce qui contourne l'obstacle de droit sur lequel le demurrage du dispositif a buté, au prix d'une incidence différente. Le quatrième est que le rendement du reflux se calcule et que personne ne l'a calculé. Le chapitre soutenait qu'un des trois facteurs était connu, le taux, que les voisins donnent à 0,5 % ; L11.C05 a établi le 2026-09-05 que ce taux porte sur une assiette différente — chaque paiement, et non la consommation finale — de sorte qu'aucun des trois facteurs n'est connu et que le corpus ne dispose d'aucune borne empirique."
concepts: [contrepartie_comptable, reflux_collectif, reflux_transactionnel, demurrage, creation_monetaire]
renvois: [L1.C18, L1.C20, L1.C21, L1.C29, L11.C01]
---

# Le trajet complet de la valeur

::etat:: Ce chapitre suit la valeur d'un bout à l'autre du circuit, dans le régime que l'arbitrage du 2026-09-05 a retenu [S1]. Il ne rouvre pas la bifurcation de L11.C01 : il l'applique, et il fait apparaître trois choses que l'examen d'une transaction isolée ne montrait pas.

## 1. Les trois étages, à l'émission

::etat:: Le Cahier Technique expose les écritures [S2], et l'arbitrage les confirme en y ajoutant la comptabilité en miroir.

::etat:: **Au Symposium.** L'émission d'allocations s'inscrit au passif. À l'actif, rien encore : le reflux n'a pas commencé. **Les fonds propres deviennent donc négatifs du montant émis.**

::etat:: **CORRECTION DE FOND DU 2026-09-07, SUR REVUE CONTRADICTOIRE.** Ce chapitre présentait ce résultat comme « la position ordinaire d'un émetteur au moment où il émet », en l'appuyant sur une analogie **qui est fausse** : « une banque centrale qui achète un titre voit son passif croître avant que le titre ne rapporte ». **Lorsqu'une banque centrale achète un titre, L'ACTIF ACQUIS ET LE PASSIF MONÉTAIRE AUGMENTENT SIMULTANÉMENT ; ses fonds propres ne baissent pas du montant émis.** Un rendement ultérieur décevant peut affecter le résultat, **mais l'achat ne crée à lui seul aucune perte.**

::hypothese:: **Le corpus enregistre que la correction ne détruit pas l'écriture décrite, elle lui retire sa justification, et c'est plus grave.** **L'écriture du Symposium reste celle-ci** — passif créé, rien à l'actif, fonds propres négatifs du montant émis. **Mais elle n'est PAS la position ordinaire d'un émetteur : elle en est l'opposé.** **Un émetteur ordinaire acquiert en émettant ; celui-ci donne en émettant.** **Ce que le chapitre présentait comme une banalité comptable est la singularité même du dispositif**, et c'est ce que L18.C01 a retrouvé par une autre voie en établissant qu'IPSAS 48 traite un tel transfert **en charge**.

::etat:: **À la banque centrale nationale.** Les allocations entrent à l'actif comme actif transitoire ; la monnaie nationale émise au bénéfice du prestataire sort au passif [S2]. L'écriture est équilibrée.

::etat:: **Chez le prestataire.** Un dépôt en monnaie nationale apparaît. Sa contrepartie — le service rendu — n'est portée à aucun bilan, puisqu'elle a été consommée par le monde. C'est ce que L1.C29 § 4 a établi et que rien ici ne modifie.

## 2. Ce que le temps fait au bilan

::etat:: La transaction isolée ne dit rien du régime. Il faut suivre plusieurs cycles.

::etat:: Les allocations sont converties, la monnaie circule, les transactions se font, le reflux collectif prélève. Sous le régime retenu, ce prélèvement **remonte** : des transactions vers la banque centrale nationale, de la banque centrale vers le Symposium, où il s'inscrit en avoirs de monnaies nationales [S1]. Les fonds propres du Symposium remontent d'autant.

::hypothese:: Trois situations sont possibles au bout d'un cycle complet, et elles ne se valent pas.

::hypothese:: **Le reflux cumulé égale l'émission cumulée.** Les fonds propres reviennent à zéro. C'est le cas d'école, et il suppose une coïncidence qu'aucun opérateur ne commande — L11.C01 § 1 l'a établi : l'émission est dimensionnée par des besoins physiques, le reflux par un comportement d'agents.

::hypothese:: **Le reflux excède l'émission.** C'est ce que l'arbitrage déclare mathématiquement possible [S1]. Le Symposium accumule. La question devient alors celle de l'emploi de cet excédent, et le § 6 y revient.

::hypothese:: **Le reflux reste inférieur à l'émission.** Les fonds propres demeurent négatifs et se creusent. Cette situation est tenable pour un émetteur — la littérature l'établit et L1.C29 § 5 l'a versée au dossier — **mais à une condition que le dispositif ne remplit pas d'évidence** : que la contrepartie accepte de porter indéfiniment le passif correspondant. Or cette contrepartie n'est pas le public : ce sont les banques centrales nationales.

## 3. Ce que la banque centrale nationale détient réellement

::etat:: Il faut regarder l'actif transitoire de plus près, parce que le mot « transitoire » y fait un travail que le régime retenu ne justifie plus.

::etat:: Sous la lecture écartée, l'adjectif était exact : les allocations refluaient vers le Symposium et s'y éteignaient, donc elles ne faisaient que passer. **Sous le régime retenu, elles ne s'éteignent pas.** Ce qui remonte au Symposium, ce sont des **avoirs en monnaies nationales**, pas les allocations. Celles-ci restent où elles sont.

::hypothese:: La banque centrale nationale porte donc à son actif, en regard d'une monnaie qu'elle a émise et qui circule dans son économie, **une créance perpétuelle sur une institution extérieure à son système, sans échéance et sans mécanisme de règlement prévu.** Le corpus doit énoncer que c'est le point le plus exposé de toute l'architecture, et il n'était visible ni dans le Cahier ni dans l'arbitrage.

::hypothese:: Une analogie éclaire, et le corpus la donne pour ce qu'elle est — la sienne, non celle des sources, et non vérifiée [S5]. Les soldes qui lient les banques centrales de l'Eurosystème entre elles ont exactement cette forme : ils naissent des paiements, n'ont pas d'échéance, et aucun mécanisme ne les règle. **Deux enseignements en découlent, opposés et également utiles.** Cela fonctionne — ces soldes existent depuis des années, à très grande échelle, sans que le système se rompe. Et cela est politiquement explosif — leur statut a nourri une controverse publique majeure, portant précisément sur ce qu'il adviendrait si une contrepartie quittait le système.

::hypothese:: La différence qui compte, et qui joue contre le dispositif : ces soldes lient des banques centrales **d'un même système**, sous un même traité et une même juridiction. La créance envisagée ici lierait une banque centrale nationale à une institution mondiale **extérieure** à son système. Ce qui rend la chose praticable dans un cas ne se transporte pas mécaniquement dans l'autre (F4, et Livre 20 lorsqu'il sera ouvert).

## 4. Où le reflux est prélevé — un point que le corpus n'avait pas posé

::etat:: Le circuit ne dit pas seulement d'où la valeur vient et où elle va : il dit **à quel nœud on la prélève**. Et ce choix n'est pas neutre.

::etat:: Le dispositif prélève **chez les agents**, au moment de la transaction pour le premier ressort, et sur les encaisses détenues pour le second. La proposition voisine la plus proche fait autrement pour le second : elle frappe « les soldes de fin de mois des **réserves bancaires déposées à la banque centrale** », à un taux de l'ordre de 0,1 % mensuel [S3].

::hypothese:: **Cette différence de nœud allège une difficulté sur laquelle le dispositif a buté — et la difficulté n'est pas celle que ce paragraphe énonçait.** Il portait, jusqu'au 2026-09-06, que « la forme la moins intrusive d'un tel registre a été jugée disproportionnée » et qu'il s'agissait d'un **obstacle de droit**. **C'est faux, et l'arrêt dit presque l'inverse** [S4] : seul l'accès **du grand public** est invalidé ; l'accès des autorités compétentes est affirmé « sans aucune restriction » (point 84), et le régime antérieur à 2018 — le moins intrusif — est déclaré « considérablement moins attentatoire » (point 85). **Douzième autocorrection du corpus** ; voir L11.C07 § 2 et A12.

::hypothese:: **Ce que la correction laisse debout, et qui suffit à l'argument de ce chapitre.** Frapper les réserves plutôt que les encaisses **écarte la question entièrement**, et pour un motif qui ne dépend d'aucune jurisprudence : les réserves sont déjà mesurées, déjà logées à la banque centrale, et **n'appartiennent à aucun particulier dont la vie privée serait en cause.** L'avantage du nœud « réserves » n'était donc pas de contourner une interdiction — il n'y en avait pas — mais de **supprimer le besoin de toute démonstration de stricte nécessité.** L'argument est plus faible qu'annoncé et il tient.

::hypothese:: Le corpus doit dire ce que la substitution coûte, faute de quoi elle paraîtrait gratuite. **L'incidence change.** Un prélèvement sur les encaisses frappe le détenteur ; un prélèvement sur les réserves frappe les banques, qui le répercutent sur leurs clients, l'absorbent sur leur marge, ou l'évitent en réduisant leurs réserves — trois comportements dont aucun n'est le comportement visé. Et l'objectif du second ressort était de décourager la thésaurisation **des agents**, non celle des banques. **La solution voisine lève un obstacle juridique en déplaçant la cible.** C'est un arbitrage, pas une amélioration gratuite, et il appartient au chapitre du demurrage de le trancher.

## 5. Ce qui se calcule, et que personne n'a calculé

::etat:: Les voisins fournissent des taux, ce que ni le livre ni le corpus ne font. Sur les stocks, 0,1 % mensuel sur des réserves de l'ordre de 4 300 milliards d'euros, soit une ressource mensuelle estimée à 4,3 milliards. Sur les flux, un micro-impôt de 0,5 % par paiement, et une taxe de 0,05 à 0,2 % sur les flux financiers [S3].

::hypothese:: Le corpus retient que **c'est la première borne empirique dont il dispose**, et qu'elle est partielle. Le rendement chiffré porte sur un seul des instruments. Celui du prélèvement sur les flux dépend d'une grandeur que personne n'avance : le volume annuel des paiements de la zone considérée, diminué de ce que les exemptions retirent — les essentiels et les régénératifs.

::hypothese:: **La forme du calcul est pourtant simple, et son absence est le vrai manque.** Rendement du reflux transactionnel = volume des paiements × part restant dans l'assiette × taux moyen pondéré par l'impact. Tant que les facteurs ne sont pas connus, ni le dispositif ni ses voisins ne peuvent dire si le reflux couvre l'émission — et c'est la formulation opérationnelle de F1. Renvoi au Livre 13, et c'est ici que la boucle courte annoncée doit passer.

::hypothese:: **CORRIGÉ le 2026-09-05, après L11.C05.** Ce paragraphe écrivait : « trois facteurs, dont un seul est connu », le taux étant donné par les voisins à 0,5 %. **Il n'est pas connu non plus.** Le taux de 0,5 % porte sur CHAQUE PAIEMENT ; l'assiette du livre est la CONSOMMATION FINALE. Ce sont deux assiettes de nature différente, et le corpus a importé un nombre de l'une vers l'autre sans conversion, puis l'a présenté comme sa seule borne empirique. **Les trois facteurs sont inconnus.** L11.C05 § 2 fournit le facteur de conversion qui manquait — le rapport du chiffre d'affaires cumulé à la valeur finale — et c'est par là que le chiffrage doit passer.

## 6. Ce que l'institution fait de ce qu'elle accumule

::etat:: L'arbitrage dit qui perçoit ; il ne dit pas ce que le percepteur en fait. Trois emplois sont ouverts, et ils ne donnent pas le même dispositif.

::hypothese:: **Dépenser.** Les avoirs financent l'émission suivante. En régime permanent, l'émission n'est plus adossée à rien de nouveau : elle est financée par le prélèvement. Le dispositif devient un budget, l'allocation devient un instrument d'amorçage, et **le Livre 19 perd sa raison d'être** puisqu'il n'y a plus de contrepartie à trouver.

::hypothese:: **Conserver.** Le Symposium constitue un fonds libellé en monnaies nationales, c'est-à-dire une créance sur les économies membres, en regard d'un passif d'allocations qui ne s'éteint pas. Ses fonds propres deviennent positifs et croissent. C'est la seule branche où l'émission reste une émission au sens fort — et c'est aussi celle où le Symposium devient un créancier mondial, ce que la thèse du dispositif combat par ailleurs (L1.C24, L1.C25).

::hypothese:: **Redistribuer.** Les avoirs retournent aux membres selon une clé. Le dispositif devient un mécanisme de transfert, et **toute la question se reporte sur la clé** — c'est-à-dire sur P31, que le corpus tient pour non écrite depuis L1.C22 § 4.

::hypothese:: Le corpus n'exprime pas de préférence. Il constate que **les trois branches déplacent la difficulté ailleurs plutôt que de la supprimer** : vers le calibrage budgétaire, vers la position de créancier, ou vers la clé de répartition. Et il relève que le statut de l'émission — sans dette ou financée par prélèvement — dépend entièrement de cette réponse, ce qui suffit à en faire la question suivante.

## 7. Portée

::etat:: Ce chapitre a suivi la valeur d'un bout à l'autre du circuit et a établi quatre choses que la transaction isolée ne montrait pas.

::etat:: **Les fonds propres de l'institution émettrice sont négatifs entre l'émission et le reflux.** **CE PARAGRAPHE AJOUTAIT, JUSQU'AU 2026-09-07, QUE C'ÉTAIT « la position ordinaire d'un émetteur et non une anomalie » ; LA REVUE CONTRADICTOIRE A ÉTABLI LE CONTRAIRE**, et le § 1 porte la correction. **Le résultat n'est pas ordinaire : il est la singularité du dispositif**, puisqu'un émetteur ordinaire acquiert un actif en émettant tandis que celui-ci donne. La condition non acquise subsiste par surcroît — que la contrepartie accepte de porter le passif.

::etat:: **Cette contrepartie porte une créance perpétuelle**, sans échéance ni mécanisme de règlement, sur une institution extérieure à son système. C'est le point le plus exposé de l'architecture, il n'apparaissait ni dans le Cahier ni dans l'arbitrage, et le corpus ne dispose pour l'éclairer que d'une analogie qu'il n'a pas vérifiée.

::etat:: **Le nœud du prélèvement est un choix de conception à part entière.** La proposition voisine la plus proche frappe les réserves bancaires là où le dispositif frappe les encaisses, ce qui lève l'obstacle de droit relevé en L1.C21 § 6 — au prix d'un déplacement de cible que le chapitre du demurrage devra trancher.

::hypothese:: **Et le rendement du reflux se calcule.** Ce n'est plus une question ouverte au sens vague : c'est un produit à trois termes, et le corpus peut désormais dire lesquels. C'est le progrès que ce chapitre revendique — non d'avoir répondu, mais d'avoir rendu la question calculable. **Le chapitre avait ajouté qu'un des trois termes était connu ; L11.C05 a établi que non, le seul chiffre disponible portant sur une autre assiette.**

## BALAYAGE DU LIVRE 21 — 2026-09-07

::etat:: **Annotation portée à l'ouverture du Livre 21, après instruction de quatre principes supplémentaires du standard des infrastructures de marché.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'écriture établie ici entre en collision frontale avec un principe normatif, et le corpus ne l'avait pas vu** [L21.C01]. **PFMI, PRINCIPE 15, RISQUE D'ENTREPRISE GÉNÉRAL** : une infrastructure doit « **hold sufficient LIQUID NET ASSETS FUNDED BY EQUITY** to cover potential general business losses **so that it can continue operations and services AS A GOING CONCERN** », et ces actifs doivent être financés par « **common stock, disclosed reserves, or other retained earnings** ».

::hypothese:: **Le principe exige des fonds propres POSITIFS ET LIQUIDES ; ce chapitre établit des fonds propres NÉGATIFS du montant émis.** **Une entité qui émet en donnant ne peut pas satisfaire un principe conçu pour une entité qui absorbe ses pertes sur son capital.**

::etat:: **La correction du 2026-09-07 avait retiré l'analogie qui présentait ce résultat comme la position ordinaire d'un émetteur.** **Ce chapitre reçoit ici la conséquence normative de cette correction** : ce n'était pas ordinaire, **et un standard international en vigueur l'interdit** à qui veut être une infrastructure.

::hypothese:: **Une issue existe, jamais écrite par le corpus : SÉPARER L'ÉMETTEUR DE L'INFRASTRUCTURE.** **Rien n'oblige l'institution qui émet à être celle qui exploite le rail.** **Mais ce qui rend l'infrastructure conforme est ce qui la rend indifférente à l'unité qu'elle transporte** — donc capable de porter la monnaie des autres.

::etat:: **Et le principe 15 exige en outre un plan de LIQUIDATION ORDONNÉE VIABLE.** **Le corpus n'a jamais écrit ce qu'il advient des unités en circulation si l'émetteur ou le rail s'arrête.**

## BALAYAGE DU LIVRE 21, SUITE — 2026-09-07

::etat:: **Annotation portée après L21.C03, qui écrit les écritures d'une opération complète entité par entité.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'écriture du § 1 est incomplète, et c'est le résultat de L21.C03.** « La monnaie nationale émise au bénéfice du prestataire sort au passif » de la banque centrale — **mais une banque centrale n'ouvre pas de compte à une entreprise ordinaire.** **Il faut une quatrième entité : la banque du prestataire**, qui reçoit les réserves à l'actif et porte le dépôt au passif. **Le § 1 de ce chapitre écrit trois étages ; il y en a quatre.**

::hypothese:: **Une conséquence en découle que le corpus pose sans l'établir, et il marque le pas.** **La partie double établit seule que le bilan de cette banque croît des deux côtés à fonds propres inchangés** — fait d'écriture, aucune source requise. **Ce qui n'est PAS établi est qu'une exigence de levier en résulte** : elle supposerait que les réserves de banque centrale entrent dans la mesure d'exposition. **Si elle se vérifiait, le coût en fonds propres du dispositif serait porté par LES BANQUES DES PRESTATAIRES, à proportion des montants émis, sans qu'on le leur ait demandé.** Acquisition déclarée en rang 1 de L21.C03.

::hypothese:: **Contrepoids porté dans le même mouvement** : toute expansion du bilan d'une banque centrale produit le même effet, et les achats d'actifs des quinze dernières années l'ont fait à une échelle sans commune mesure. **L'objection porte donc sur un coût et un calibrage, non sur une impossibilité.**

::etat:: **Et le § 3 reçoit son nom normatif.** La « créance perpétuelle, sans échéance et sans mécanisme de règlement » qu'il décrit est, dans le vocabulaire du **PFMI principe 8**, une obligation **sans date de valeur** : « final settlement, **at a minimum by the end of the value date** ». **Le caractère définitif est acquis gratuitement sur la jambe qui paie le prestataire, et il est impossible sur la jambe qui lie l'émetteur à la banque centrale.**

## BALAYAGE DU LIVRE 21, TROISIÈME — 2026-09-07

::etat:: **Annotation portée après L21.C04, qui ferme l'acquisition sur le ratio de levier de Bâle par lecture directe du texte.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **L'HYPOTHÈSE POSÉE PAR L'ANNOTATION PRÉCÉDENTE EST VÉRIFIÉE, ET DANS LE SENS DÉFAVORABLE** [L21.C04]. **Cadre consolidé de Bâle, LEV30.8** : « **Banks must include ALL BALANCE SHEET ASSETS in their leverage ratio exposure measure** » — **aucune exception pour les réserves de banque centrale.** **LEV20.7** : « **Banks must meet a 3% leverage ratio minimum requirement AT ALL TIMES** », en capital de première catégorie [LEV20.4]. **Et la version d'origine de 2014, fouillée intégralement, ne mentionne PAS UNE SEULE FOIS les banques centrales**, tout en écrivant que « **netting of loans and deposits is NOT ALLOWED** ».

::hypothese:: **Donc l'exigence de fonds propres que l'émetteur ne peut pas satisfaire [L21.C01] ne disparaît pas : elle SE DÉPLACE vers les banques des prestataires.** **Trois de capital de première catégorie par cent d'unités en circulation** — **première grandeur d'ordre que le corpus obtienne du côté de l'émission, et c'est un RATIO, donc il croît à la vitesse de l'ambition.**

::etat:: **La seule dérogation se referme sur elle-même.** **LEV30.7** autorise une exemption **temporaire**, en **circonstances macroéconomiques exceptionnelles**, pour **faciliter la politique monétaire** — **et impose à la juridiction qui l'utilise d'augmenter le minimum « COMMENSURATELY to offset the impact ».** **Elle relocalise le coût, elle ne le supprime pas.**

::hypothese:: **PORTÉE BORNÉE, ET IL FAUT LA DIRE.** **CRE20.7 pondère à 0 % les expositions sur les banques centrales** : les réserves ne consomment **aucun** capital pondéré. **Le levier est la seule contrainte qui les atteigne, et il ne mord que pour les banques déjà à leur limite.** **Le coût est réel, non uniforme, et il frappe les plus grandes** — celles qui portent en outre le coussin de LEV40.

## BALAYAGE DU LIVRE 21, CINQUIÈME — 2026-09-07

::etat:: **Annotation portée après L21.C06, qui ouvre la directive de 1998 sur le caractère définitif du règlement.** Ce que ce chapitre reçoit **ne modifie pas ce qui précède** : il est versé pour que la passe 2 le trouve.

::etat:: **Ce que la protection ajoute à la créance perpétuelle du § 3, et ce qu'elle n'y ajoute pas** [L21.C06]. **Directive 98/26/CE, article 3 § 1** : les ordres de transfert « produisent leurs effets en droit et, **même en cas de procédure d'insolvabilité à l'encontre d'un participant, sont opposables aux tiers** » s'ils ont été introduits avant l'ouverture.

::hypothese:: **Le besoin de protection est ASYMÉTRIQUE, et l'asymétrie est défavorable.** **Une banque centrale émettrice de sa propre monnaie ne fait pas défaut dans cette monnaie** ; **le seul participant susceptible de faire défaut est l'institution émettrice.** **Donc la partie que la désignation protégerait est LA BANQUE CENTRALE NATIONALE, et le risque couvert est LE DÉFAUT DU SYMPOSIUM.**

::etat:: **La créance reste perpétuelle ; ce que la désignation ajoute est que LES TRANSFERTS DÉJÀ FAITS NE SE DÉFONT PAS.** **C'est peu, et c'est le minimum sans lequel aucune banque centrale n'entrerait** — sans désignation, une banque centrale ayant renvoyé des allocations vers un émetteur ensuite liquidé n'a aucune protection particulière contre le liquidateur.

::hypothese:: **Le dispositif a donc besoin de la désignation NON POUR SA PROPRE SÉCURITÉ, MAIS POUR ÊTRE ACCEPTABLE À SES CONTREPARTIES** — qui sont exactement les institutions qu'il doit convaincre d'adhérer.
