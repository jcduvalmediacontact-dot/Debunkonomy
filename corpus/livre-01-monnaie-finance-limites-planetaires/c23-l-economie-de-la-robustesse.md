---
chapitre: L1.C23
titre: "L'économie de la robustesse"
livre: 1
langue: fr
...
verifications_en_attente:
  ...
  # « Livre 1 | 23 | L'économie de la robustesse ».
  #
  # PÉRIMÈTRE — Ce chapitre ne refait pas ce que L1.C04 § 5 a établi : que le
  # déplacement d'objectif de la performance vers la robustesse est proposable,
  # et que la robustesse n'est pas absente de l'économie (contrôle robuste,
  # décision robuste en incertitude profonde, résilience territoriale) mais n'y
  # figure pas comme objectif d'ensemble. L1.C04 § 7 avait laissé ouverte une
  # question précise — le rééquilibrage vers la robustesse passe-t-il
  # nécessairement par une modification de l'émission monétaire ? Le dispositif
  # ayant été exposé en L1.C17 à L1.C22, ce chapitre y revient.
  #
  # ── Audit contradictoire du 2026-09-04 — traitement ────────────────────────
  # Onze objections, toutes retenues. Deux méritent d'être signalées comme des
  # défaillances du corpus et non comme des divergences d'appréciation.
  #
  # 1. BLOQUANTE, ACCEPTÉE — Ressources réelles en transition (§ 3). Le chapitre
  #    déduisait qu'il faut émettre davantage pendant la transition sans voir
  #    que c'est le moment où l'éviction réelle mord le plus fort. Or le corpus
  #    avait établi cette objection la veille, en L1.C22 § 5 et en P34(a). Ce
  #    n'est donc pas une lacune, c'est une INCOHÉRENCE INTERNE : un chapitre
  #    ignorant un résultat de son voisin immédiat. Le § 3 porte désormais les
  #    deux contraintes opposées et les trois conditions qui en découlent.
  # 2. ACCEPTÉE — Portée de Jackson et Victor (§ 4). Un résultat d'existence
  #    obtenu sous hypothèses restrictives était présenté comme établissant le
  #    caractère conditionnel de la contrainte. Les hypothèses sont désormais
  #    nommées, et la position adverse ajoutée [S10] : le débat porte sur
  #    l'origine de l'impératif, non sur son existence. Ce que le corpus retient
  #    se réduit à ceci — la contrainte n'est pas ARITHMÉTIQUE.
  # 3. ACCEPTÉE — Définition de la stabilité financière (§ 1). Le Cahier la
  #    lisait comme la préservation d'un état figé ; les définitions de
  #    référence contiennent déjà une composante de résilience [S9]. Le corpus
  #    corrige la lecture du Cahier et déplace la critique sur ce qui la
  #    soutient : le critère de succès reste indexé sur la rentabilité.
  # 4. ACCEPTÉE — Zones monétaires optimales (§ 5). Objection ignorée, et
  #    d'autant plus pertinente que les chocs visés sont écologiques, donc
  #    asymétriques par nature, alors que le taux de conversion est fixe. Elle
  #    constitue la TROISIÈME route vers la même objection de centralisation.
  # 5. ACCEPTÉE — Capture de rente sur la redondance financée (§ 2). Une
  #    capacité redondante et une capacité sous-employée sont indiscernables de
  #    l'extérieur ; les distinguer est une décision administrative, donc une
  #    rente [S13]. Ajouté au § 2, en lien avec P28.
  # 6. ACCEPTÉE — Endogénéité minskyenne (§ 6). Les agents adaptent leur
  #    comportement à la robustesse perçue : la robustesse construite d'un côté
  #    est consommée de l'autre. Ajouté avec sa source [S12].
  # 7. ACCEPTÉE — « Paradoxe de la tranquillité » sans source. Rattaché à Minsky
  #    [S12], avec la précision que la formule n'est pas de lui.
  # 8. ACCEPTÉE — « Un système non éprouvé est indiscernable d'un système
  #    fragile » : affirmation non sourcée et trop forte. Reformulée — un tel
  #    système n'est pas fragile, il est NON RENSEIGNÉ.
  # 9. à 11. ACCEPTÉES — Marqueurs de régime. § 1 (définition) ::norme:: ->
  #    ::etat:: ; § 4 (positionnement du corpus) et § 7 (conclusion analytique)
  #    ::etat:: -> ::hypothese::.
  #
  # ── Sourçage documentaire du 2026-09-04 — traitement ───────────────────────
  # Les quatre demandes de sourçage sont closes, et deux apportent plus que ce
  # qui était demandé. Les citations reproduites dans les sources en viennent ;
  # elles restent des PISTES tant qu'un humain n'a pas ouvert les textes.
  #
  # S4 — CLOS. Citations exactes de Holling aux p. 17 et 21, y compris l'énoncé
  #   de la corrélation inverse (« a system can be very resilient and still
  #   fluctuate greatly, i.e. have low stability ») et le passage sur le
  #   rendement maximal soutenu qui détruit la résilience qu'il stabilise.
  # S5 et S6 — CLOS, et la borne bilatérale est confirmée : la robustesse
  #   fonctionnelle est un produit NON MONOTONE dont l'extremum se situe à une
  #   valeur intermédiaire d'ascendance, non à un maximum d'efficacité. La
  #   citation de S6 sur la diversité et l'interconnexion est reproduite : elle
  #   soutient directement l'objection du § 5.
  # S9 — CLOS, ET LE RAPPORT SE CONTREDIT. Sa fiche cite le texte de la Banque
  #   centrale européenne — « capable of withstanding shocks and the unravelling
  #   of financial imbalances » — puis son commentaire affirme que
  #   « l'institution conçoit la stabilité comme le retour à l'équilibre d'une
  #   architecture inchangée ». Le texte cité dit l'inverse du commentaire : il
  #   contient bien une composante de résilience. **Le corpus suit la citation,
  #   contre le commentaire** — ce qui confirme la correction déjà faite au § 1
  #   après l'audit contradictoire. C'est la deuxième fois qu'un rapport
  #   documentaire se contredit ainsi entre sa fiche et sa synthèse (cf. L1.C22,
  #   statut comptable des DTS) : lire les fiches, se méfier des synthèses.
  # S7 — CLOS, et l'énoncé des auteurs est PLUS FORT que ce que le corpus en
  #   tirait. Le § 4 le cite désormais avant de le borner. DOI ajouté ; le
  #   modèle se nomme FALSTAFF.
  # S8 — CLOS. Identifiants et chapitres pertinents ajoutés.
  # CHIFFRES DE 2020 — RÉSOLUS, et la confusion du Cahier est expliquée. Les
  #   2 300 Md$ du 9 avril 2020 sont un PLAFOND de facilités de prêt, tiré à une
  #   fraction ; l'expansion CONSTATÉE du bilan est de 2 344 Md$ entre le 11 mars
  #   et le 29 avril, et elle procède d'achats de titres, non de ces facilités.
  #   Deux grandeurs distinctes que leur voisinage numérique confond. Le § 1 les
  #   emploie désormais en les séparant explicitement [S14].
  #
  # RÉSERVE — Le rapport prête au chapitre une position qu'il ne tient pas :
  # que « le Livre 1 ne traite que du régime permanent ». Le chapitre dit
  # l'inverse — le régime transitoire décide de la viabilité de l'ensemble et
  # n'a été examiné nulle part (§ 3, P35). Ce n'est pas une délimitation
...
::hypothese:: Cette précision ne fragilise pas la position du corpus : elle la renforce, et par le côté qu'on n'attendait pas. Ce que L1.C08 § 2 nommait la condition d'accumulation — la contrainte apparaît lorsque les créanciers accumulent au lieu de dépenser — cesse d'être une hypothèse commode pour devenir un fait documenté et chiffré. **Les économies observées satisfont la condition sous laquelle la contrainte existe.** Le débat théorique reste ouvert ; le cas empirique, lui, penche.
...
::hypothese:: C'est la troisième proposition qui porte l'argument du corpus, et elle ne demande pas que la deuxième soit tranchée. Le texte du livre, lui, s'appuie sur la version forte : c'est là que passe la divergence.

::hypothese:: La version que le corpus soutient est donc plus étroite, et elle suffit : dans les conditions effectivement observées, le service d'un encours croissant exige des revenus croissants ; et le régime en vigueur plafonne l'une des deux manières de s'en dispenser, l'érosion par l'inflation, et traite l'autre, le défaut, comme un accident à prévenir (L1.C12 § 5). Ce n'est pas une nécessité mécanique, c'est une configuration institutionnelle — ce qui la rend d'ailleurs modifiable, alors que la version forte la rendrait fatale.

::hypothese:: Le corpus enregistre cette divergence comme une divergence, non comme une nuance : sur ce point, le texte du livre affirme plus que ce que le corpus tient pour établi.
...
## 6. Une économie de la robustesse n'a pas d'indicateur
...
::hypothese:: Mais l'objectif de robustesse ajoute une difficulté que les indicateurs alternatifs ordinaires ne traitent pas. Mesurer un état — la surface forestière, l'espérance de vie en bonne santé — n'est pas mesurer une **capacité à encaisser une perturbation non anticipée**. La robustesse est une propriété conditionnelle : elle ne se manifeste que lorsque le choc survient. Un système qui n'a pas été éprouvé n'est pas pour autant fragile ; il est **non renseigné**, et ses indicateurs de bon fonctionnement ne disent rien de ce qu'il ferait sous contrainte.

::etat:: Le mécanisme qui rend cette difficulté active est documenté. Une période prolongée de stabilité conduit les agents à réduire leurs marges de sécurité et à accroître leur levier, de sorte que la stabilité engendre les conditions de son propre renversement [S12] — c'est la forme que prend, dans la sphère financière, la corrélation inverse posée au § 1.
...
::etat:: Il en résulte que « piloter vers la robustesse » suppose un instrument qui n'existe pas, et que les objections déjà posées sur la métrologie écosystémique (L1.C18 § 5) valent ici avec une difficulté supplémentaire. Le corpus renvoie l'examen des indicateurs à L1.C28, en notant que ce renvoi ne vaudra pas réponse s'il se borne à énumérer des mesures d'état.
