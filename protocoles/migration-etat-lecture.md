...
| **E-M1** | manifeste **absent, illisible ou incohérent** — identifiant en doublon, `etat_initial` autre que `a_requalifier`, `date_migration` absente, mal formée ou différente de celle de l'en-tête, nombre déclaré différent du nombre d'entrées. **Le manifeste est inutilisable** : E-L6 n'est pas évaluable, les autres contrôles s'exécutent, et la publication est refusée |
